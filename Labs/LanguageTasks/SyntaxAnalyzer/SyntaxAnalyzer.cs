...
namespace SyntaxAnalyzer
{
	...
	public class SyntaxAnalyzer
	{
		...
		private bool IsWhileStatement(List<Lexeme> lexemeList)
		{
			...
			if (!_lexemeEnumerator.MoveNext() || _lexemeEnumerator.Current.Type != LexemeType.Do) { Error("Ожидается do", _lexemeList.IndexOf(_lexemeEnumerator.Current)); }
			_lexemeEnumerator.MoveNext();

			if (_lexemeEnumerator.Current == null || _lexemeEnumerator.Current.Type != LexemeType.While) { Error("Ожидается while", _lexemeList.IndexOf(_lexemeEnumerator.Current)); }
